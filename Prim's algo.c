...
u=from[v]; spanning[u][v]=distance[v];
spanning[v][u]=distance[v]; no_of_edges--; visited[v]=1;
...
for(j=0;j<n;j++)
{
printf("\n Enter the value for G[%d][%d]=",i,j); scanf("%d",&G[i][j]);
}
}
...
printf("\n\nTotal minimum cost of spanning tree = %d",total_cost);
              return 0;
}
